...
class CSVReportCreator(ReportCreator):
    ...
    def create(self, data: GitStatistics, path: str, config: dict):
        ...
        first_commit = datetime.datetime.fromtimestamp(data.first_commit_timestamp).strftime(date_time_format)
        last_commit = datetime.datetime.fromtimestamp(data.last_commit_timestamp).strftime(date_time_format)
        general_info = dict()
        general_info['1'] = {
            'Project name': self.project_name,
            'Repo name': data.repo_name,
            'Generated date': datetime.datetime.now().strftime(date_time_format),
